...
void configOptions(EmuEnv* _env, Options *op, BlockBasedTableOptions *t_op, WriteOptions *w_op, ReadOptions *r_op, FlushOptions *f_op) {
    ...
    std::cout << "Memtable Factory Set To = " << _env->memtable_factory << std::endl; // !AS-Feb08-XXIV!
...
int runWorkload(EmuEnv* _env) {
  ...
  if (options.compaction_style == kCompactionStyleUniversal)
    ...
    std::cout << "Compaction Eagerness: 1-Leveling (variation of kCompactionStyleLeveling)" << std::endl; // !YBS-apr06-XXI! 
  else
  ...
  if (_env->compaction_pri == 9) {
    ...
    // std::cout << "Compaction style = 1-Leveling :: Overloading level0_file_num_compaction_trigger to " << _env->level0_file_num_compaction_trigger;
    // std::cout << "Compaction style = 1-Leveling :: Overloading level0_stop_writes_trigger to " << _env->level0_stop_writes_trigger;
  }
  // !END
  
  // time variables for measuring the time taken by the workload
  std::chrono::time_point<std::chrono::high_resolution_clock> start, end;
  std::chrono::time_point<std::chrono::high_resolution_clock> insert_start, insert_end;
  std::chrono::time_point<std::chrono::high_resolution_clock> query_start, query_end;
  std::chrono::time_point<std::chrono::high_resolution_clock> delete_start, delete_end;
  ...
  while(!workload_file.eof()) {
    ...
    case 'R': // range delete 
      ...
      s = db->DeleteRange(w_options,std::to_string(start_key),std::to_string(end_key));
      assert(s.ok());
  ...
  std::cout << "Total time taken by updates = " << total_update_time_elapsed.count() << " seconds" << std::endl;
  std::cout << "Total time taken by deletes = " << total_delete_time_elapsed.count() << " seconds" << std::endl;
  std::cout << "Total time taken by range deletes = " << total_range_delete_time_elapsed.count() << " seconds" << std::endl;
  std::cout << "Total time taken by range queries = " << total_range_query_time_elapsed.count() << " seconds" << std::endl;
  ...
  std::cout << "Re-opening DB -- Re-setting compaction style to: " << _env->compaction_pri << "\n";
...
*/
  ...
  args::ValueFlag<int> memtable_factory_cmd(group1, "memtable_factory", "[Memtable Factory: 1 for Skiplist, 2 for Vector, 3 for Hash Skiplist, 4 for Hash Linkedlist; def: 3]", {'m', "memtable_factory"}); // !!AS-feb08-XXIV!!
  ...
  args::ValueFlag<double> del_per_th_cmd(group1, "del_per_th", "Delete persistence threshold [def: -1]", {'t', "dpth"}); // !YBS-feb15-XXI!
  args::ValueFlag<int> enable_rocksdb_perf_iostat_cmd(group1, "enable_rocksdb_perf_iostat", "Enable RocksDB's internal Perf and IOstat [def: 0]", {"stat"}); // !YBS-feb15-XXI!

  args::ValueFlag<long> num_inserts_cmd(group1, "inserts", "The number of unique inserts to issue in the experiment [def: 1]", {'i', "inserts"});
  ...
  _env->memtable_factory = memtable_factory_cmd ? args::get(memtable_factory_cmd) : 3;
  ...
  _env->delete_persistence_latency = del_per_th_cmd ? args::get(del_per_th_cmd) : 0; // !YBS-feb15-XXI!
...
void printExperimentalSetup(EmuEnv* _env) {
  ...
  // std::cout << std::setfill(' ') << std::setw(l) << _env->delete_persistence_latency; // !YBS-feb15-XXI!
