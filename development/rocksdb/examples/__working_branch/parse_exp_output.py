...
dummy_values = ['99'] #these values should be ignored. This is a bug in c++ cout.
for line in Lines: 
    ...
    if prev_line_first_word == "cmpt_sty" or prev_line_first_word == "#p_ts_in_tree" or prev_line_first_word == "%space_amp": #if previous line begins with one of these
        ...
        if prev_line_first_word == "#p_ts_in_tree":
            i = len(temp_list)
            while i != 12:
                temp_list.extend(dummy_values)
                i+=1
                
        if prev_line_first_word == "%space_amp":
            i = len(temp_list)
            while i != 3:
                temp_list.extend(dummy_values)
                i+=1
